...
- 空间复杂度：![O(\logm+n) ](./p__O_log_m_+_n__.png) ，哈希表 ![\textit{rank} ](./p__textit{rank}_.png)  需要的空间为 *O(n)*，排序需要的栈空间为 ![O(\logm) ](./p__O_log_m__.png) 。
